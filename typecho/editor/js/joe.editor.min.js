$(function () {
  function t(t) {
    ...
    switch (t) {
      ...
      case "wmd-anote-button":
        s = `{anote icon="Font Awesome图标" href="跳转地址" type="secondary|success|warning|error|info（选择其一）"}${
          $("#text").selectionRange() ? $("#text").selectionRange() : "便条按钮"
        }{/anote}`;
        break;
      case "wmd-abtn-button":
        s = `{abtn icon="Font Awesome图标" color="颜色值" href="跳转地址" radius="圆角（最大17.5px）"}${
  ...
  s.forEach((s) => {
    let e = $(
      `<li class="wmd-button" id="${s.id}" title="${s.title}">${s.svg}</li>`
    );
    e.on("mousedown", () => $("#text").insertContent(t(s.id))),
      $("#wmd-button-row").append(e);
  }),
    $.ajax({
      ...
      success(t) {
        let s = "",
          e = "";
        for (let c in t)
          (s += `<div class="item" data-index="${t[c].index}">${c}</div>`),
            (e += `\n                    <ul class="scroll" data-index="${
              t[c].index
            }">\n                        ${t[c].container
              .map(
                (t) => `<li class="item" data-text="${t.data}">${t.icon}</li>`
              )
              .join("")} \n                    </ul>\n                `);
        $("#wmd-button-row").append(
          ...
          $(".joe_owo__contain .seat").on("click", function (t) {
            t.stopPropagation(), $(this).siblings(".box").toggleClass("show");
          }),
          $(".joe_owo__contain .box .bar .item").on("click", function (t) {
            t.stopPropagation(),
            ...
            const s =
              ...
              '"]';
            $(s).show().siblings(".scroll").hide();
          }),
          $(".joe_owo__contain .scroll .item").on("click", function () {
            const t = $(this).attr("data-text");
            $("#text").insertContent(t);
          }),

$(function () {
  $("#wmd-editarea textarea").attr(
    "placeholder",
    "请输入文章内容...（开启粘贴上传功能后，支持图片粘贴上传哦~）"
  );
  ...
  $("#wmd-fullscreen-button").on("click", function () {
    $(".fullscreen #text").css(
      "top",
      $(".fullscreen #wmd-button-bar").outerHeight()
    );
  });
  /* 增加自定义功能 */
  const items = [
  ...
  function getInsertTextById(id) {
    ...
    switch (id) {
      ...
      case "wmd-code-button":
        str = ` \`${
          $("#text").selectionRange() ? $("#text").selectionRange() : "行内代码"
        }\` `;
        break;
      ...
      case "wmd-delete-button":
        str = ` ~~${
          $("#text").selectionRange() ? $("#text").selectionRange() : "删除线"
        }~~ `;
        break;
      ...
      case "wmd-copy-button":
        str = `{copy text="默认文本"}${
          $("#text").selectionRange() ? $("#text").selectionRange() : "点击复制"
        }{/copy}`;
        break;
      case "wmd-mtitle-button":
        str = `\n{mtitle}${
          $("#text").selectionRange() ? $("#text").selectionRange() : "居中标题"
        }{/mtitle}\n`;
        break;
      case "wmd-message-button":
        str = `\n{message type="success|info|warning|error（选择其一）"}${
          $("#text").selectionRange() ? $("#text").selectionRange() : "提示内容"
        }{/message}\n`;
        break;
      case "wmd-anote-button":
        str = `{anote icon="Font Awesome图标" href="跳转地址" type="secondary|success|warning|error|info（选择其一）"}${
          $("#text").selectionRange() ? $("#text").selectionRange() : "便条按钮"
        }{/anote}`;
        break;
      case "wmd-abtn-button":
        str = `{abtn icon="Font Awesome图标" color="颜色值" href="跳转地址" radius="圆角（最大17.5px）"}${
          $("#text").selectionRange() ? $("#text").selectionRange() : "多彩按钮"
        }{/abtn}`;
        break;
  ...
  items.forEach((_) => {
    let item = $(
      `<li class="wmd-button" id="${_.id}" title="${_.title}">${_.svg}</li>`
    );
    item.on("mousedown", () =>
      $("#text").insertContent(getInsertTextById(_.id))
    );

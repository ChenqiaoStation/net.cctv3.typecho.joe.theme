document.addEventListener("DOMContentLoaded", () => {
    ...
    constructor() {
      ...
      this.options = {
        ...
        type: /^secondary$|^success$|^warning$|^error$|^info$/.test(
          ...
          : "secondary",
        content:
          this.innerHTML.trim().replace(/^(<br>)|(<br>)$/g, "") || "标签按钮",
      };
      this.render();
    }
    get template() {
  ...
  class JoeAbtn extends HTMLElement {
    constructor() {
      ...
      this.options = {
        ...
        content:
          this.innerHTML.trim().replace(/^(<br>)|(<br>)$/g, "") || "多彩按钮",
      };
      this.render();
    }
    get template() {
  ...
  class JoeDplayer extends HTMLElement {
    ...
    get template() {
      return `
                <iframe class="joe_detail__article-player" allowfullscreen="true" src="${
  ...
  class JoeMtitle extends HTMLElement {
    constructor() {
      ...
      this.options = {
        content:
          this.innerHTML.trim().replace(/^(<br>)|(<br>)$/g, "") || "居中标题",
      };
      this.render();
    }
    get template() {
  ...
  class JoeCopy extends HTMLElement {
    constructor() {
      ...
      this.options = {
        ...
        content:
          this.innerHTML.trim().replace(/^(<br>)|(<br>)$/g, "") || "点击复制",
      };
      this.render();
    }
    get template() {
    ...
    constructor() {
      ...
      this.options = {
        type: /^success$|^info$|^warning$|^error$/.test(
          ...
          : "info",
        content:
          this.innerHTML.trim().replace(/^(<br>)|(<br>)$/g, "") || "提示内容",
      };
      this.render();
    }
    get template() {
  ...
  class JoeCard extends HTMLElement {
    constructor() {
      ...
      this.options = {
        ...
        content:
          this.innerHTML.trim().replace(/^(<br>)|(<br>)$/g, "") || "默认内容",
      };
      this.render();
    }
    get template() {
  ...
  class JoeShow extends HTMLElement {
    constructor() {
      ...
      this.options = {
        content:
          this.innerHTML.trim().replace(/^(<br>)|(<br>)$/g, "") ||
          "默认隐藏的内容",
      };
      this.render();
    }
  ...
  class JoeHide extends HTMLElement {
    ...
    event() {
      ...
      this.$button.addEventListener("click", () => {
        const top =
          $(".joe_comment").offset().top - $(".joe_header").height() - 15;

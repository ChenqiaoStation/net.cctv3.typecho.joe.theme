document.addEventListener("DOMContentLoaded", () => {
  ...
  function initVideoList() {
    ...
    $.ajax({
      ...
      success(res) {
        if (res.code !== 1)
          return $(".joe_video__type-list").html(
            `<li class="error">${res.data}</li>`
          );
        ...
        res.data.class.forEach(
          (_) =>
            (htmlStr += `<li class="item animated swing" data-t="${_.type_id}">${_.type_name}</li>`)
        );
    ...
    function renderDom() {
      ...
      $.ajax({
        ...
        success(res) {
          if (res.code !== 1)
            return $(".joe_video__list-item")
              .css("display", "block")
              .html('<p class="error">数据加载失败！请检查！</p>');
          if (!res.data.list.length) {
            $(".joe_video__list-item")
              .css("display", "block")
              .html('<p class="error">暂无数据！</p>');
          } else {
            let htmlStr = "";
            res.data.list.forEach((_) => {
              htmlStr += `
								<a class="item animated bounceIn" href="${
                  window.location.href + "?vod_id=" + _.vod_id
                }" target="_blank" rel="noopener noreferrer nofollow">
									<i class="year" style="display: ${
                    _.vod_year && _.vod_year != 0 ? "block" : "none"
                  }">${_.vod_year}</i>
									<div class="thumb">
										<img width="100%" height="100%" class="pic lazyload" src="${
                      Joe.LAZY_LOAD
                    }" data-src="${
                _.vod_pic
              }" onerror="javascript: this.src = '${Joe.LAZY_LOAD}'" alt="${
                _.vod_name
              }">
									</div>    
    ...
    function initPagination() {
      ...
      if (queryData.pg != 1) {
        htmlStr += `
            		...
            		<li class="joe_video__pagination-item" data-pg="${
                  queryData.pg - 1
                }">
      ...
      if (queryData.pg < pagecount)
        htmlStr += `<li class="joe_video__pagination-item" data-pg="${pagecount}">末页</li>`;
      $(".joe_video__pagination").html(htmlStr);
    }
    $(".joe_video__pagination").on(
      "click",
      ".joe_video__pagination-item",
      function () {
    ...
    $(".joe_video__list-search .input").on(
      "keyup",
      (e) => e.keyCode === 13 && searchFn()
    );
  }
  function initVideoDetail() {
    ...
    $.ajax({
      ...
      success(res) {
        if (res.code !== 1)
          return $(".joe_video__detail-info").html(
            `<p class="error">${res.data}</p>`
          );
        if (!res.data.list.length)
          return $(".joe_video__detail-info").html(
            `<p class="error">数据抓取异常！请检查！</p>`
          );
        ...
        $(".joe_video__detail-info").html(`
					<div class="thumbnail">
						<img width="100%" height="100%" class="pic lazyload" src="${
              Joe.LAZY_LOAD
            }" data-src="${item.vod_pic}" onerror="javascript: this.src = '${
          Joe.LAZY_LOAD
        }'" alt="${item.vod_name}">
						<i class="year" style="display: ${
              item.vod_year && item.vod_year != 0 ? "block" : "none"
            }">${item.vod_year}</i>
					</div>
					<dl class="description">
						<dt>${item.vod_name + (item.vod_remarks ? " - " + item.vod_remarks : "")}</dt>
						<dd><span class="muted">类型：</span><span class="text">${
              item.vod_class || "未知"
            }</span></dd>
						<dd><span class="muted">主演：</span><span class="text">${
              item.vod_actor || "未知"
            }</span></dd>
						<dd><span class="muted">导演：</span><span class="text">${
              item.vod_director || "未知"
            }</span></dd>
						<dd><span class="muted">简介：</span><span class="text">${getContent(
              item
            )}</span></dd>
					</dl>
				`);
        /* 设置视频播放标题 */
        $(".joe_video__player .joe_video__contain-title").html(
          "正在播放：" + item.vod_name
        );
        /* 设置播放链接 */
        let parseList = (str) => {
        ...
        let maps = new Map();
        playFromArr.forEach((element, index) =>
          maps.set(element, playUrlArr[index] || [])
        );
        let htmlStr = "";
        let index = 0;
        ...
        $(".joe_video__source")
          .first()
          .find(".joe_video__source-list .item")
          .first()
          .click();
      },
    });
    $(document).on("click", ".joe_video__source-list .item", function () {
      ...
      const offset =
        $(".joe_video__player").offset().top - $(".joe_header").height() - 15;

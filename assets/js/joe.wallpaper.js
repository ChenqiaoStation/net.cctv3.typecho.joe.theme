document.addEventListener("DOMContentLoaded", () => {
  ...
  $.ajax({
    ...
    success(res) {
      if (res.code !== 1)
        return $(".joe_wallpaper__type-list").html(
      ...
      res.data.forEach(
        (_) =>
          (htmlStr += `<li class="item animated swing" data-cid="${_.id}">${_.name}</li>`)
      );
      ...
      success(res) {
        ...
        res.data.forEach((_) => {
          htmlStr += `
            ...
            _.img_1024_768 || _.url
          }" onerror="javascript: this.src = '${Joe.LAZY_LOAD}'" alt="壁纸">
                        </a>`;
        });
  ...
  function initPagination() {
    ...
    if (queryData.start / queryData.count !== 0) {
      htmlStr += `
                ...
                <li class="joe_wallpaper__pagination-item" data-start="${
                ...
                <li class="joe_wallpaper__pagination-item" data-start="${
                  queryData.start - queryData.count
                }">${Math.ceil(queryData.start / queryData.count)}</li>
            `;
    }
    htmlStr += `<li class="joe_wallpaper__pagination-item active">${
    ...
    if (queryData.start != total - queryData.count) {
      htmlStr += `
                <li class="joe_wallpaper__pagination-item" data-start="${
                  queryData.start + queryData.count
                }">${Math.ceil(queryData.start / queryData.count) + 2}</li>
                <li class="joe_wallpaper__pagination-item" data-start="${
                  queryData.start + queryData.count
                }">
    ...
    if (queryData.start < total - queryData.count)
      htmlStr += `<li class="joe_wallpaper__pagination-item" data-start="${
        total - queryData.count
      }">末页</li>`;
    $(".joe_wallpaper__pagination").html(htmlStr);
  }
  $(".joe_wallpaper__pagination").on(
    "click",
    ".joe_wallpaper__pagination-item",
    function () {

document.addEventListener("DOMContentLoaded", () => {
  const e = (e) => window.btoa(unescape(encodeURIComponent(e))),
    o = (e) => decodeURIComponent(escape(window.atob(e))),
    t = $(".joe_detail").attr("data-cid");
  $.ajax({
    url: Joe.BASE_API,
    type: "POST",
  ...
  {
    let a = localStorage.getItem(e("views"))
      ? JSON.parse(o(localStorage.getItem(e("views"))))
      : [];
    ...
    n ||
      $.ajax({
        url: Joe.BASE_API,
        type: "POST",
        data: { routeType: "handle_views", cid: t },
        success(o) {
          if (1 !== o.code) return;
          $("#Joe_Article_Views").html(`${o.data.views} 阅读`), a.push(t);
          const n = e("views"),
  ...
  }
  {
    ...
    const o = [
      ...
      a = $(".joe_detail__leaving-list").width(),
      n = $(".joe_detail__leaving-list").height(),
      i = [
        "20px 300px",
        "20px 400px",
        "20px 500px",
    ...
    $(".joe_detail__leaving-list .item").each((c, s) => {
      const d = t(1, 99),
        r = o[t(0, o.length - 1)],
        p = Math.ceil($(s).width()),
      ...
      $(s).css({
        ...
        $(s).draggabilly({ containment: !0 }),
        $(s).on("dragStart", (o) => {
          e++, $(s).css({ zIndex: e });
        });
    });
  }
  $(".joe_comment .joe_pagination a").each((e, o) => {
    const t = $(o).attr("href");
    t &&
      t.includes("#") &&
      $(o).attr("href", t.replace("#comments", "?scroll=joe_comment"));
  });
}),
  window.addEventListener("load", function () {
    {
      ...
      if (e) {
        const o = $(".joe_header").height();
        let t = null;
        if (
          ((t = $("#" + e).length > 0 ? $("#" + e) : $("." + e)),
          t && t.length > 0)
        ) {
          const e = t.offset().top - o - 15;

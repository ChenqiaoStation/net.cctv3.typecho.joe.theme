document.addEventListener("DOMContentLoaded", () => {
  ...
  /* 激活侧边栏排行榜功能 */
  {
    if ($(".joe_aside__item.ranking").length !== 0) {
      $.ajax({
        ...
        success(res) {
          $(".joe_aside__item.ranking .joe_aside__item-title .text").html(
            res.title
          );
          let htmlStr = "";
          if (res.code === 1) {
            res.data.forEach((item, index) => {
              htmlStr += `
									<li class="item">
										...
										<a class="link" href="${item.url}" title="${
                item.title
  ...
  /* 激活Live2d人物 */
  {
    if (Joe.LIVE2D !== "off" && Joe.LIVE2D) {
      $.getScript(
        "http://www.cctv3.net/MyCDN/assets/L2Dwidget.min.js",
        () => {
          L2Dwidget.init({
  ...
  /* 评论框点击切换画图模式和文本模式 */
  {
    $(".joe_comment__respond-type .item").on("click", function () {
      ...
      if ($(this).attr("data-type") === "draw") {
        ...
        $("#joe_comment_draw").prop(
          "width",
          $(".joe_comment__respond-form .body").width()
        );
        /* 设置表单格式为画图模式 */
  ...
  /* 激活画图功能 */
  {
    if ($("#joe_comment_draw").length !== 0) {
      ...
      /* 撤销上一步 */
      $(".joe_comment__respond-form .body .draw .icon-undo").on("click", () =>
        window.sketchpad.undo()
      );
      /* 动画预览 */
      $(".joe_comment__respond-form .body .draw .icon-animate").on(
        "click",
        () => window.sketchpad.animate(10)
      );
      /* 更改画板的线宽 */
      $(".joe_comment__respond-form .body .draw .line li").on(
        "click",
        function () {
      ...
      /* 更改画板的颜色 */
      $(".joe_comment__respond-form .body .draw .color li").on(
        "click",
        function () {
  ...
  /* 激活评论提交 */
  {
    ...
    $(".joe_comment__respond-form").on("submit", function (e) {
      e.preventDefault();
      const url =
        $(".joe_comment__respond-form").attr("action") + "?time=" + +new Date();
      const type = $(".joe_comment__respond-form").attr("data-type");
      const parent = $(".joe_comment__respond-form").attr("data-coid");
      const author = $(
        ".joe_comment__respond-form .head input[name='author']"
      ).val();
      const mail = $(
        ".joe_comment__respond-form .head input[name='mail']"
      ).val();
      let text = $(
        ".joe_comment__respond-form .body textarea[name='text']"
      ).val();
  ...
  /* 初始化表情功能 */
  {
    if ($(".joe_owo__contain").length > 0 && $(".joe_owo__target").length > 0) {
      $.ajax({
        url: "http://www.cctv3.net/facebook/Joe.json",
        success(res) {
          let barStr = "";
          let scrollStr = "";
          for (let key in res) {
            ...
            scrollStr += `
                            <ul class="scroll" data-index="${res[key].index}">
                                ${res[key].container
                                  .map(
                                    (_) =>
                                      `<li class="item" data-text="${_.data}">${_.icon}</li>`
                                  )
                                  .join("")} 
                            </ul>
                        `;
          ...
          $(".joe_owo__contain .box .bar .item").on("click", function (e) {
            ...
            $(this).addClass("active").siblings().removeClass("active");
            const scrollIndx =
              '.joe_owo__contain .box .scroll[data-index="' +
